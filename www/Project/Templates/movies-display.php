...
<?php
                 $notInCart = false;
                if($logged){
                    $cart = CartQuery::create()->findByUserId($_SESSION["id"]); 
                    foreach($cart as $c){
	        ...
	        if($logged){
		    if($notInStock){
			echo "<div class ='alert alert-danger'> Not In Stock</div>";
		    }	    
		    else if(!$notInCart){
...
?>
                ...
                <div class="modal fade" id="<?php echo $p->getId()."modal"; ?>">
					...
					if($logged){
					    if($notInStock){
						echo "<button type ='sumbit' class ='btn btn-danger' disabled> Not In Stock</button>";
					    }	    
					    else if(!$notInCart){
